...
router.post('/testdrive-permission', authMiddleware , async(req, res) => {
    ...
    await newtestdrive.save();
     ...
     unseenNotification.push({
        ...
        message: `${newtestdrive.name} has requested for test drive`,
        data:{
        testdriveId : newtestdrive._id,
        name : newtestdrive.name,
    },
    onClickPath : "/admin/testdrive"
     })
    ...
    res.status(200).send({
        ...
        console.log(error)
        res.status(500).send({ message: "Error in applyying for test drive", success: false, error});
    }
})
